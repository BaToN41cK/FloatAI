...
// Заголовки секций, которые включаются ВСЕГДА — это «скелет» личности.
const ALWAYS_SECTIONS = new Set(['Личность', 'Приоритеты инструкций', 'Контекст о пользователе', 'Формат и принципы']);
...
const SECTION_KEYWORDS = {
  ...
  'Проверка роли': ['кто ты', 'ты кто', 'характер', 'себя вести', 'правила'],
  'Контекст о пользователе': ['меня зовут', 'мой имя', 'меня интересует', 'мои интересы', 'моя цель']
};
...
// файл личности целиком. Никакой выборки секций под запрос: агент всегда помнит
// всё (характер, интересы, контекст о пользователе, правила) в каждом сообщении.
